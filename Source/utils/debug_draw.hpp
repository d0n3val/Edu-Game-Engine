...
void arc(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx, ) ddVec3_In center, ddVec3_In planeNormal, ddVec3_In color,
	const float radius, const float numSteps, float minAngle, float maxAngle, const int durationMillis, const bool depthEnabled)
{
	if (!isInitialized(DD_EXPLICIT_CONTEXT_ONLY(ctx)))
	{
		return;
	...
	{
		const float radians = minAngle+ (angleSize * i / numSteps);
