...
ts::DeBoorNet::DeBoorNet(const ts::DeBoorNet& other)
{
	...
	assert(err >= 0); // check err code or ts_enum_str(err) for more info
}
...
const ts::rational* ts::DeBoorNet::result_ptr() const
{
...
void ts::BSpline::setCtrlp(const ts::rational* ctrlp)
{
	assert(ctrlp);
    ...
    const tsError err = ts_bspline_set_ctrlp(&bspline, ctrlp, &bspline);
	assert(err >= 0); // check err code or ts_enum_str(err) for more info
}

void ts::BSpline::setCtrlp(const std::vector<ts::rational> ctrlp)
{
	assert(ctrlp.size() == nCtrlp() * dim());
    // "The number of values must be equals to the"
    // " spline's number of control points multiplied by the dimension"
    // " of each control point."
